...
if st.session_state.cod_municipio_AM:
    ...
    with st.container():
        ...
        valores = [int(dados[0][0]), int(dados[0][1])]

        col1, col2, col3 = st.columns(3)
        with col1:
            st.write("Empenho x Receita")
        ...
        with col2:
            st.markdown("<p style='color: red; font-weight: bold'>Empenho x Receita</p>", unsafe_allow_html=True)
        ...
        with col3:
            ...
            st.markdown("<p style='color: blue; font-weight: bold'>Empenho x Receita</p>", unsafe_allow_html=True)
            # st.write("Empenho x Receita")
            ...
            bar_chart = alt.Chart(source).mark_arc(innerRadius=50).encode(
                # theta="Valores em R$",
                # color="Tipo:N",
                theta=alt.Theta(field="Valores em R$", type="quantitative", stack=True, scale=alt.Scale(type="linear",rangeMax=1.5708, rangeMin=-1.5708 )),
